    ...
    /* USER CODE BEGIN 3 */
	  ...
	  /* 每 100 ms 切換一次 LED */
	      ...
	      if (uart_rx_ready)
	      {
	          ...
	          if ((length > 0U) && (length <= UART_BUFFER_SIZE))
	          {
	              /*
	               * 使用獨立 TX Buffer，避免重新接收時
	               * DMA RX 覆寫 DMA TX 正在讀取的資料。
	               */
	              memcpy(uart_tx_buffer, uart_rx_buffer, length);
