...
void HardFault_C(uint32_t *fault_stack)
{
    /*
     * Cortex-M 硬體自動保存的 Exception Stack Frame。
     */
    fault_r0   = fault_stack[0];
    fault_r1   = fault_stack[1];
    fault_r2   = fault_stack[2];
    fault_r3   = fault_stack[3];
    fault_r12  = fault_stack[4];
    fault_lr   = fault_stack[5];
    fault_pc   = fault_stack[6];
    fault_xpsr = fault_stack[7];

    /*
     * System Control Block 的 Fault Status Registers。
     */
    fault_cfsr  = SCB->CFSR;
    fault_hfsr  = SCB->HFSR;
    fault_bfar  = SCB->BFAR;
