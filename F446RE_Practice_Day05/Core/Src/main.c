...
volatile uint32_t queue_drop_count = 0;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    if (GPIO_Pin == GPIO_PIN_13)
    {
        ...
        /*
         * ISR 中 timeout 必須設為 0，
         * 因為 ISR 不能等待 Queue 空間。
         */
...
void StartButtonTask(void *argument)
{
  /* USER CODE BEGIN StartButtonTask */
	...
	const uint32_t debounce_ticks =
		(osKernelGetTickFreq() * 200U) / 1000U;

  /* Infinite loop */
	for (;;)
	{
		/*
		 * 沒有事件時，Task 會進入 Blocked 狀態，
		 * 不會持續占用 CPU。
		 */
				...
				osWaitForever) == osOK)
		{
			 ...
			 * 使用差值運算可處理 Tick 回繞。
			 */
			if ((uint32_t)(now - last_press_tick) >=
				debounce_ticks)
			{
				last_press_tick = now;
				button_press_count++;
