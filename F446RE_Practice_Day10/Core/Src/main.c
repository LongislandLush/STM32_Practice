...
#define STACK_WARNING_BYTES 128U
...
volatile uint32_t worker_stack_free_bytes = 0U;
volatile uint32_t worker_loop_count = 0U;
volatile uint32_t worker_checksum = 0U;
...
{
    /*
     * volatile 避免編譯器把整個陣列最佳化掉。
     * 這個陣列會占用 WorkerTask 的 Stack。
     */
    volatile uint8_t work_buffer[1784];
...
void StartMonitorTask(void *argument)
{
	    ...
	    {
	        /*
	         * CMSIS-RTOS2 回傳目前曾經剩下的最小 Stack 空間，
	         * 單位通常為 Bytes。
	         */
	        worker_stack_free_bytes =
	            osThreadGetStackSpace(WorkerTaskHandle);

	        if (worker_stack_free_bytes <
	            STACK_WARNING_BYTES)
	        {
	            HAL_GPIO_WritePin(
	                GPIOA,
	        ...
	        int length = snprintf(
	            message,
	            sizeof(message),
	            "Worker loop=%lu, checksum=%lu, "
	            "minimum stack free=%lu bytes\r\n",
	            (unsigned long)worker_loop_count,
	            (unsigned long)worker_checksum,
	            (unsigned long)worker_stack_free_bytes);
